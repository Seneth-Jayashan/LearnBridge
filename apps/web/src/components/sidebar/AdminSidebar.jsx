...
const AdminSidebar = ({ isOpen, close, isExpanded, setIsExpanded }) => {
  ...
  const links = [
    ...
    { name: "Manage Levels", path: "/admin/levels", icon: FiBook, dropdown: levelsOpen, setDropdown: setLevelsOpen, submenus: [{ label: "Add Level", path: "/admin/levels/add" }, { label: "Manage Levels", path: "/admin/levels/manage" }] },
    { name: "Manage Grades", path: "/admin/grades", icon: FiAward, dropdown: gradesOpen, setDropdown: setGradesOpen, submenus: [{ label: "Add Grade", path: "/admin/grades/add" }, { label: "Manage Grades", path: "/admin/grades/manage" }] },
    { name: "Manage Modules", path: "/admin/modules", icon: FiLayers, dropdown: modulesOpen, setDropdown: setModulesOpen, submenus: [{ label: "Add Module", path: "/admin/modules/add" }, { label: "Manage Modules", path: "/admin/modules/manage" }] },
          ...
          <div className="flex items-center gap-3 overflow-hidden w-full">
            ...
            {isExpanded && (
              <div className="flex flex-col animate-in fade-in duration-300">
                ...
                <span className="text-[10px] font-bold text-[#4CAF50] tracking-widest uppercase">Workspace</span>
              </div>
            )}
          </div>
        </div>
            ...
            if (link.submenus) {
              return (
                <div key={link.name} className={`relative transition-all duration-200 group ${isExpanded ? "px-5" : ""}`}>
                  <button
                    onClick={() => link.setDropdown(!link.dropdown)}
                    className={`w-full flex items-center gap-4 py-3.5 transition-all duration-200 outline-none text-left ${isExpanded ? "" : "justify-center"} ${isActive ? "bg-linear-to-r from-[#207D86]/20 to-transparent text-white" : "text-slate-400 hover:bg-white/5 hover:text-white"}`}
                  >
                    ...
                    {isExpanded && <FiChevronRight className={`ml-auto transition-transform ${link.dropdown ? "rotate-90 text-white" : "text-slate-500"}`} />}
                  ...
                  {isExpanded && link.dropdown && (
                    <div className="mt-1 pl-9 flex flex-col gap-1 animate-in fade-in slide-in-from-top-2 duration-200">
                      {link.submenus.map((item) => {
                        const isSubActive = location.pathname === item.path;
                        return (
                          <Link key={item.path} to={item.path} onClick={close} className={`relative flex items-center gap-3 py-2 px-3 rounded-lg text-sm transition-all ${isSubActive ? "bg-white/10 text-white" : "text-slate-400 hover:bg-white/5 hover:text-white"}`}>
                            {isSubActive && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-4 bg-[#4CAF50] rounded-r-full" />}
