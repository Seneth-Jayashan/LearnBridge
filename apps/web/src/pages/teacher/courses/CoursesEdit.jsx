...
const CoursesEdit = () => {
  ...
  useEffect(() => {
    const loadCourse = async () => {
      try {
        ...
        const course = Array.isArray(data) ? data.find((item) => item._id === id) : null;
        if (!course) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setError(firstValidationMessage || err.response?.data?.message || backendError || "Failed to update course");
    } finally {
  ...
  return (
    <section className="max-w-5xl mx-auto space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-[#0E2A47]">Edit Course</h2>
        <p className="text-slate-600 mt-1">Update your course details.</p>
      </div>
      ...
      {isLoading ? (
        <div className="bg-white border border-slate-200 rounded-xl p-5 text-slate-600">Loading course...</div>
      ) : (
        <form onSubmit={handleSubmit} className="bg-white border border-slate-200 rounded-xl p-5 space-y-4">
          <div className="grid md:grid-cols-2 gap-4">
            <div>
              <label htmlFor="name" className="block text-sm font-semibold text-slate-700 mb-1">Course Title</label>
              <input id="name" name="name" value={formData.name} onChange={handleInputChange} className="w-full border border-slate-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#207D86]" />
            </div>

            <div>
              <label htmlFor="shortDescription" className="block text-sm font-semibold text-slate-700 mb-1">Short Description</label>
              <input id="shortDescription" name="shortDescription" value={formData.shortDescription} onChange={handleInputChange} className="w-full border border-slate-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#207D86]" />
            </div>
            ...
            <div>
              ...
              <input id="durationHours" name="durationHours" type="number" min="0" step="0.5" value={formData.durationHours} onChange={handleInputChange} className="w-full border border-slate-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#207D86]" />
            </div>

            <div>
              <label htmlFor="numberOfLessons" className="block text-sm font-semibold text-slate-700 mb-1">Number of Lessons</label>
              <input id="numberOfLessons" name="numberOfLessons" type="number" min="0" step="1" value={formData.numberOfLessons} onChange={handleInputChange} className="w-full border border-slate-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#207D86]" />
            </div>

            <div>
              <label htmlFor="instructor" className="block text-sm font-semibold text-slate-700 mb-1">Instructor / Creator</label>
              <input id="instructor" name="instructor" value={loggedInstructorName || formData.instructor} readOnly className="w-full border border-slate-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#207D86]" />
            </div>
          </div>

          <div>
            <label htmlFor="fullDescription" className="block text-sm font-semibold text-slate-700 mb-1">Full Description</label>
            <textarea id="fullDescription" name="fullDescription" value={formData.fullDescription} onChange={handleInputChange} rows={4} className="w-full border border-slate-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#207D86]" />
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button type="button" onClick={() => navigate("/teacher/courses/manage")} className="px-4 py-2 rounded-lg border border-slate-300 text-slate-700">
              Cancel
            </button>
          </div>
        </form>
