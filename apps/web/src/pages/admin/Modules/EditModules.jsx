...
const EditModules = () => {
	...
	if (isLoading) {
		return <div className="bg-white border border-slate-200 rounded-xl p-5 text-slate-600">Loading module...</div>;
	}

	return (
		<section className="max-w-6xl mx-auto space-y-6">
			<div>
				<h2 className="text-2xl font-bold text-[#0E2A47]">Edit Module</h2>
				<p className="text-slate-600 mt-1">Update module details by level, grade, and stream rules.</p>
			</div>
			...
			<form onSubmit={handleSubmit} className="bg-white border border-slate-200 rounded-xl p-5 space-y-4">
				<div className="grid md:grid-cols-2 gap-4">
					<div>
						<label htmlFor="level" className="block text-sm font-semibold text-slate-700 mb-1">Level</label>
						<select id="level" name="level" value={formData.level} onChange={handleInputChange} className="w-full border border-slate-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#207D86]">
					...
					<div>
						<label htmlFor="grade" className="block text-sm font-semibold text-slate-700 mb-1">Grade</label>
					...
					{isAdvanced && (
						<div className="md:col-span-2">
							<label htmlFor="subjectStream" className="block text-sm font-semibold text-slate-700 mb-1">Subject Stream</label>
							<select id="subjectStream" name="subjectStream" value={formData.subjectStream} onChange={handleInputChange} className="w-full border border-slate-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#207D86]">
					...
					<div>
						...
						<input id="name" name="name" value={formData.name} onChange={handleInputChange} className="w-full border border-slate-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#207D86]" placeholder="Algebra Basics" />
					</div>
					...
					<div className="md:col-span-2">
						...
						<textarea id="description" name="description" rows={3} value={formData.description} onChange={handleInputChange} className="w-full border border-slate-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#207D86]" placeholder="Short module summary" />
					</div>
					...
					{formData.level && formData.grade && recommendations.length > 0 && (
						<div className="md:col-span-2 border border-slate-200 rounded-lg p-4 bg-slate-50">
							<p className="text-sm font-semibold text-slate-800 mb-2">{getRecommendationTitle(gradeNumber)}</p>
							<p className="text-xs text-slate-600 mb-3">Suggested module names while editing (not a dropdown):</p>
							<div className="flex flex-wrap gap-2">
								{recommendations.map((subject) => (
									<span key={subject} className="px-2.5 py-1 rounded-full bg-white border border-slate-300 text-xs text-slate-700">
										{subject}
									</span>
								))}
							</div>
				...
				<div className="flex items-center gap-3">
					...
					<button type="button" onClick={() => navigate("/admin/modules/manage")} className="inline-flex items-center px-4 py-2 rounded-lg border border-slate-300 text-slate-700 font-semibold hover:bg-slate-50">
						Cancel
					</button>
				</div>
			</form>
